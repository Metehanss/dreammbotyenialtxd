...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `\`${this.name}\` Komut sadece bot sahibi tarafından kullanılabilir.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `\`${this.name}\` Komutunu kullanmak için aşağıdaki izinlere ihtiyacınız var\n"${permissions[missing[0]]}"`;
				}
				return oneLine`
					\`${this.name}\` Komutunu kullanmak için aşağıdaki izinlere ihtiyacınız var:
	...
	reload() {
		...
		try {
			...
			try {
				...
				if(cached) require.cache[cmdPath] = cached;
				if(err2.message.includes('Modül bulunamadı')) throw err; else throw err2;
			}
		}
	...
	unload() {
		...
		if(!require.cache[cmdPath]) throw new Error('Komut kaldırılamaz.');
		delete require.cache[cmdPath];
	...
	static validateInfo(client, info) { // eslint-disable-line complexity
		...
		if(typeof info !== 'object') throw new TypeError('Komut bilgisi bir Nesne olmalı.');
		if(typeof info.name !== 'string') throw new TypeError('Komut adı bir dize olmalı.');
		if(info.name !== info.name.toLowerCase()) throw new Error('Komut adı küçük harf olmalı');
		if(info.aliases && (!Array.isArray(info.aliases) || info.aliases.some(ali => typeof ali !== 'string'))) {
			throw new TypeError('Komut takma adları dizeleri bir dizi olmalıdır.');
		}
		if(info.aliases && info.aliases.some(ali => ali !== ali.toLowerCase())) {
			throw new Error('Command aliases must be lowercase.');
		}
		if(typeof info.group !== 'string') throw new TypeError('Komut grubu bir dize olmalı.');
		if(info.group !== info.group.toLowerCase()) throw new Error('Komut grubu küçük harf olmalıdır.');
		if(typeof info.memberName !== 'string') throw new TypeError('Komut üyesiAdı bir dize olmalı.');
		if(info.memberName !== info.memberName.toLowerCase()) throw new Error('Komut üyesi adı küçük harf olmalı.');
		if(typeof info.description !== 'string') throw new TypeError('Komut açıklaması bir dize olmalı.');
		if('format' in info && typeof info.format !== 'string') throw new TypeError('Komut biçimi bir dize olmalı.');
		if('details' in info && typeof info.details !== 'string') throw new TypeError('Komut ayrıntıları bir dize olmalı.');
		if(info.examples && (!Array.isArray(info.examples) || info.examples.some(ex => typeof ex !== 'string'))) {
			throw new TypeError('Komut örnekleri dizge dizisi olmalıdır.');
		}
		if(info.clientPermissions) {
			if(!Array.isArray(info.clientPermissions)) {
				throw new TypeError('Komut istemcisi İzinleri, izin anahtarı dizelerinin bir Dizisi olmalıdır.');
			}
			for(const perm of info.clientPermissions) {
				if(!permissions[perm]) throw new RangeError(`Geçersiz komut istemcisi izni: ${perm}`);
			}
		}
		if(info.userPermissions) {
			if(!Array.isArray(info.userPermissions)) {
				throw new TypeError('Komut kullanıcısı İzinler, izin anahtarı dizelerinin bir dizisi olmalıdır.');
			}
			for(const perm of info.userPermissions) {
				if(!permissions[perm]) throw new RangeError(`Geçersiz komut kullanıcısı İzin: ${perm}`);
			}
		}
		if(info.throttling) {
		...
		if(info.argsType && !['single', 'multiple'].includes(info.argsType)) {
			throw new RangeError('Geçersiz seçim "single" olmalı yada "multiple" şekilde olmalı.');
		}
